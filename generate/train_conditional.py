...
#             raise ValueError("xformers is not available. Make sure it is installed correctly")
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            labels = batch["label"].long()  # Convert labels to tensor and move to appropriate device
        ...
        if accelerator.is_main_process:
            if epoch % args.save_images_epochs == 0 or epoch == args.num_epochs - 1:
                ...
                eval_labels = torch.arange(num_classes, device=pipeline.device, dtype=torch.long).repeat(
                    (eval_bs + num_classes - 1) // num_classes
                )[:eval_bs]
                images = pipeline(
            generator=generator,
            ...
            output_type="np",
                ...
                save_images_locally(images_processed, "generate/saved_images", epoch)
                if args.logger == "tensorboard":
